...
def upload_file():
	if request.method == 'POST':
		...
		if file and allowed_file(file.filename):
                  ...
                  df = pd.read_excel(os.path.join(UPLOAD_FOLDER+"/"+session['username']+"/input"+filename)
                  preprocess_text(df,"review")
                  #flash('File successfully uploaded')
                  return redirect(url_for('home'))
		else:
			flash('Allowed file types are txt,xlsx, csv')
			return redirect(url_for('home'))
                

def preprocess(filename):
    df = pd.read_excel(os.path.join(UPLOAD_FOLDER+"/"+session['username']+"/input"+filename)
    features_file, actions_file, feelings_file, noun_adj_file = preprocess_text(df,"review")
    features_file.to_excel(os.path.join(UPLOAD_FOLDER+"/"+session['username']+"/output/features_file.xlsx", index=False)
	flash('features_file successfully uploaded')
